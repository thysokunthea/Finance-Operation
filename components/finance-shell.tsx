        ...
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside className="relative flex h-full w-[min(86vw,320px)] flex-col bg-sidebar text-sidebar-foreground shadow-2xl">
            ...
            <nav aria-label="Mobile navigation" className="flex-1 overflow-y-auto px-3 py-5">
              ...
              <div className="space-y-1">{workflowNav.map((item) => { const Icon = item.icon; return <a key={item.key} href={item.href} onClick={() => setMobileOpen(false)} className="flex h-10 items-center gap-3 rounded-lg px-3 text-sm text-sidebar-foreground/72 transition-colors hover:bg-sidebar-accent/65 hover:text-white"><Icon className="size-4" /><span>{item.label}</span>{item.count ? <span className="ml-auto rounded-full bg-amber-400/15 px-2 py-0.5 text-[10px] font-semibold text-amber-300">{item.count}</span> : null}</a>; })}</div>
            </nav>
          </aside>
        </div>
      ) : null}
