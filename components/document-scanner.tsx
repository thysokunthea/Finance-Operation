...
export function DocumentScanner({ onClose, onApply }: { onClose: () => void; onApply: (result: ScannedTransaction) => void }) {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/55 p-4 backdrop-blur-sm">
    ...
    <section aria-label="Scan transaction document" className="relative z-10 max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-2xl border bg-card shadow-2xl">
      <header className="flex items-start justify-between border-b p-5"><div className="flex gap-3"><span className="grid size-10 place-items-center rounded-xl bg-primary/10 text-primary"><ScanLine className="size-5" /></span><div><h2 className="text-xl font-semibold">Intelligent document scan</h2><p className="mt-1 text-xs text-muted-foreground">PNG, JPG/JPEG, or PDF · maximum 10 MB · reads up to 3 PDF pages</p></div></div><Button type="button" variant="ghost" size="icon" aria-label="Close scanner" onClick={onClose}><X /></Button></header>
      <div className="space-y-5 p-5">
        ...
        {file ? <div className="flex flex-col gap-3 rounded-xl border p-3 sm:flex-row sm:items-center"><span className="grid size-9 place-items-center rounded-lg bg-muted text-primary">{file.type === 'application/pdf' ? <FileText className="size-4" /> : <FileImage className="size-4" />}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{file.name}</p><p className="text-xs text-muted-foreground">{file.type} · {(file.size / 1024).toFixed(0)} KB</p></div><Button type="button" onClick={scan} disabled={progress > 0 && progress < 100}>{progress > 0 && progress < 100 ? <Loader2 className="animate-spin" /> : <ScanLine />} {progress > 0 && progress < 100 ? 'Scanning…' : 'Scan document'}</Button></div> : null}
        ...
        {result ? <div className="space-y-4 rounded-2xl border border-emerald-200 bg-emerald-50/45 p-4">
          ...
          <div className="flex justify-end gap-2"><Button type="button" variant="outline" onClick={() => { setResult(null); setRawText(''); setProgress(0); }}>Scan again</Button><Button type="button" onClick={() => onApply(result)}>Review and record</Button></div>
        </div> : null}
      </div>
    </section>
  </div>;
