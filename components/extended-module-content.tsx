...
function Requests() {
  ...
  const createRequest = () => setItems((current) => current.some((item) => item[0] === 'REQ-2026-0199') ? current : [['REQ-2026-0199', 'New expense request', 'Operations', '$0', 'Draft'], ...current]);
  return <div className="space-y-5"><Header crumb="Finance / Payment Requests" title="Payment requests" copy="Submit, review, authorize, pay, and close requests in one controlled workflow." action={<Button onClick={createRequest}>New request</Button>} /><Metrics values={[["$26.7K","Pending value"],[String(items.length),"Requests in queue"],["96%","On-time processing"]]} /><ListCard title="Request queue" description="Approval levels are selected from configured amount rules">{items.map((r) => <Row key={r[0]} title={r[1]} subtitle={`${r[0]} · ${r[2]}`} value={r[3]} status={r[4]} />)}</ListCard></div>;
}

function Budgets() {
  return <div className="space-y-5"><Header crumb="Finance / Budgets" title="Budget control" copy="Monitor actual and committed spending before limits are exceeded." action={<Button variant="outline" onClick={() => { window.location.href = '/settings'; }}>Edit thresholds</Button>} /><Metrics values={[["$365K","Monthly budget"],["$248.2K","Actual spending"],["$85.4K","Remaining"]]} /><ListCard title="Department utilization" description="Actual and committed spend compared with August budget">{budgetLines.map((r) => <div key={r[0]} className="grid gap-3 border-b p-4 last:border-0 md:grid-cols-[1fr_120px_120px_120px_90px] md:items-center"><div><p className="text-sm font-medium">{r[0]}</p><Progress value={Number(r[4].replace('%',''))} className="mt-2 h-1.5" /></div><Value label="Budget" value={r[1]} /><Value label="Actual" value={r[2]} /><Value label="Committed" value={r[3]} /><Badge variant="outline" className={r[4] === '87%' ? 'border-amber-200 bg-amber-50 text-amber-700' : 'border-emerald-200 bg-emerald-50 text-emerald-700'}>{r[4]} used</Badge></div>)}</ListCard></div>;
}
