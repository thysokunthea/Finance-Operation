...
const blankDraft = (type: 'All' | 'Income' | 'Expense'): TransactionDraft => ({
  type: type === 'All' ? 'Expense' : type, documentType: 'Manual entry', date: todayInputDate(), dueDate: '',
...
export function TransactionsContent({ type = 'All' }: { type?: 'All' | 'Income' | 'Expense' }) {
  ...
  const [scannerMode, setScannerMode] = useState<'upload' | 'camera' | null>(null);
  ...
  const filtered = useMemo(() => typeRows.filter((row) => `${row.id} ${row.reference} ${row.party} ${row.category}`.toLowerCase().includes(query.toLowerCase()) && (status === 'All statuses' || row.status === status) && (departmentFilter === 'All departments' || row.department === departmentFilter) && isWithinDateRange(row.date, dateFrom, dateTo)), [typeRows, query, status, departmentFilter, dateFrom, dateTo]);
  ...
  const scopeMismatch = type !== 'All' && typeRows.length === 0 && records.length > 0;
  const emptyTitle = records.length === 0 ? 'No transactions in this register' : scopeMismatch ? `${records.length} saved transaction${records.length === 1 ? '' : 's'} found outside ${type.toLowerCase()}` : 'No matching transactions';
  const emptyCopy = records.length === 0 ? 'Create, scan, or OCR-import a transaction.' : scopeMismatch ? 'Open All Transactions to review the saved records and their transaction types.' : 'Adjust or clear the active filters.';
  ...
  const openEditTransaction = (record: TransactionRow) => {
    ...
    setDraft({
      ...
      date: toInputDate(record.date), dueDate: toInputDate(record.dueDate || ''), reference: record.reference, party: record.party,
  ...
  const saveTransaction = async (event: React.FormEvent) => {
    ...
    setSaving(true); setNotice('Saving transaction…');
    try {
  ...
  const useScannedData = (scanned: ScannedTransaction) => {
    ...
    setEditingId(null); setDraft({ ...fields, type: type === 'All' ? fields.type : type, date: toInputDate(fields.date) || todayInputDate(), dueDate: toInputDate(fields.dueDate), department: suggestDepartment(fields.category, fields.description) }); setScannerMode(null); setNotice('Scanned fields were auto-filled. Review highlighted values, then save.'); setDialogOpen(true);
  };

  const resetFilters = () => {
    const range = currentMonthRange();
    setQuery(''); setStatus('All statuses'); setDepartmentFilter('All departments'); setDateFrom(range.from); setDateTo(range.to);
  };

  const updateSubtotal = (value: string) => {
    const previousCalculated = sumDraft(draft.subtotal, draft.tax);
    setDraft({ ...draft, subtotal: value, amount: !draft.amount || draft.amount === previousCalculated ? sumDraft(value, draft.tax) : draft.amount });
  };

  const updateTax = (value: string) => {
    const previousCalculated = sumDraft(draft.subtotal, draft.tax);
    setDraft({ ...draft, tax: value, amount: !draft.amount || draft.amount === previousCalculated ? sumDraft(draft.subtotal, value) : draft.amount });
  };
  ...
  return <div className="space-y-5">
    ...
    {dialogOpen ? <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/55 p-4 backdrop-blur-sm"><button type="button" aria-label="Close transaction dialog" className="absolute inset-0" onClick={() => setDialogOpen(false)} /><form aria-label={editingId ? 'Edit transaction form' : 'New transaction form'} onSubmit={saveTransaction} className="relative z-10 max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-2xl border bg-card shadow-2xl"><div className="flex items-start justify-between border-b p-5"><div><h2 className="text-xl font-semibold">{editingId ? `Edit ${editingId}` : 'Review transaction'}</h2><p className="mt-1 text-xs text-muted-foreground">{editingId ? 'Changes return the record to Pending finance review and remain traceable.' : 'Confirm scanned values. Saving creates a Pending record for finance review.'}</p></div><Button type="button" variant="ghost" size="icon" aria-label="Close dialog" onClick={() => setDialogOpen(false)}><X /></Button></div><div className="grid gap-4 p-5 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <Field label="Issue date"><Input required type="date" min="2020-01-01" max="2030-12-31" value={draft.date} onChange={(event) => setDraft({ ...draft, date: event.target.value })} className="mt-2" /></Field>
      <Field label="Due date"><Input type="date" min={draft.date || '2020-01-01'} max="2030-12-31" value={draft.dueDate} onChange={(event) => setDraft({ ...draft, dueDate: event.target.value })} className="mt-2" /></Field>
      ...
      <Field label="Subtotal"><Input type="number" min="0" step="0.01" value={draft.subtotal} onChange={(event) => updateSubtotal(event.target.value)} className="mt-2" /></Field>
      <Field label="Tax / VAT"><Input type="number" min="0" step="0.01" value={draft.tax} onChange={(event) => updateTax(event.target.value)} className="mt-2" /></Field>
    ...
    </div><div className="flex justify-end gap-2 border-t p-5"><Button type="button" variant="outline" onClick={() => setDialogOpen(false)} disabled={saving}>Cancel</Button><Button type="submit" disabled={saving}>{saving ? 'Saving…' : editingId ? 'Save changes for review' : 'Save as Pending'}</Button></div></form></div> : null}
  </div>;
}
...
function isWithinDateRange(value: string, from: string, to: string) { const timestamp = Date.parse(value); if (!Number.isFinite(timestamp)) return true; if (from && timestamp < Date.parse(`${from}T00:00:00`)) return false; if (to && timestamp > Date.parse(`${to}T23:59:59`)) return false; return true; }
function formatPickerDate(value: string) { if (!value) return ''; const date = new Date(`${value}T00:00:00`); return new Intl.DateTimeFormat('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }).format(date); }
function todayInputDate() { const date = new Date(); return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`; }
function currentMonthRange() { const date = new Date(); const year = date.getFullYear(); const month = date.getMonth(); return { from: `${year}-${String(month + 1).padStart(2, '0')}-01`, to: `${year}-${String(month + 1).padStart(2, '0')}-${String(new Date(year, month + 1, 0).getDate()).padStart(2, '0')}` }; }
function toInputDate(value: string) { if (!value) return ''; if (/^\d{4}-\d{2}-\d{2}$/.test(value)) return value; const parsed = new Date(value); if (!Number.isFinite(parsed.getTime())) return ''; return `${parsed.getFullYear()}-${String(parsed.getMonth() + 1).padStart(2, '0')}-${String(parsed.getDate()).padStart(2, '0')}`; }
function sumDraft(subtotal: string, tax: string) { const total = Number(subtotal || 0) + Number(tax || 0); return Number.isFinite(total) && total > 0 ? total.toFixed(2) : ''; }
function suggestDepartment(category: string, description: string) { const value = `${category} ${description}`.toLowerCase(); if (/software|cloud|hosting|technology|internet|it\b/.test(value)) return 'Technology'; if (/marketing|advertis|campaign|design/.test(value)) return 'Marketing'; if (/revenue|sale|customer|service income/.test(value)) return 'Commercial'; return 'Operations'; }
