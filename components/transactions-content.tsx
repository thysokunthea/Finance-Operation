...
export function TransactionsContent({ type = 'All' }: { type?: 'All' | 'Income' | 'Expense' }) {
  ...
  const [dateScope, setDateScope] = useState<'August' | 'All'>('August');
  ...
  const filtered = useMemo(() => typeRows.filter((row) => `${row.id} ${row.reference} ${row.party} ${row.category}`.toLowerCase().includes(query.toLowerCase()) && (status === 'All statuses' || row.status === status) && (departmentFilter === 'All departments' || row.department === departmentFilter)), [typeRows, query, status, departmentFilter]);
  ...
  return <div className="space-y-5">
    ...
    <Card className="gap-0 shadow-[0_1px_2px_rgb(15_23_42/3%)]"><CardContent className="p-3"><div className="flex flex-col gap-2 md:flex-row"><div className="relative min-w-0 flex-1"><Search className="absolute left-2.5 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search ID, reference, counterparty or category" className="h-9 pl-8" /></div><select value={status} onChange={(event) => setStatus(event.target.value)} className="h-9 rounded-lg border bg-card px-3 text-sm outline-none"><option>All statuses</option><option>Paid</option><option>Pending</option><option>Approved</option><option>Overdue</option><option>Missing Document</option></select><Button variant="outline" className="h-9" onClick={() => setDateScope((current) => current === 'August' ? 'All' : 'August')}><CalendarDays /> {dateScope === 'August' ? '1–31 Aug 2026' : 'All dates'}</Button><Button variant="outline" className="h-9" onClick={() => setShowAdvanced((current) => !current)} aria-expanded={showAdvanced}><Filter /> {showAdvanced ? 'Hide filters' : 'More filters'}</Button></div>{showAdvanced ? <div className="mt-3 flex flex-col gap-2 border-t pt-3 sm:flex-row sm:items-center"><label className="text-xs font-medium text-muted-foreground">Department</label><select value={departmentFilter} onChange={(event) => setDepartmentFilter(event.target.value)} className="h-9 rounded-lg border bg-card px-3 text-sm"><option>All departments</option><option>Operations</option><option>Commercial</option><option>Technology</option><option>Marketing</option></select><Button variant="ghost" className="sm:ml-auto" onClick={() => { setQuery(''); setStatus('All statuses'); setDepartmentFilter('All departments'); setDateScope('August'); }}>Reset filters</Button></div> : null}</CardContent></Card>
